...
from database.models import User, LearningSession, EEGSession, Recommendation
...
async def get_eeg_analytics(
    ...
    try:
        ...
        peak_times = [
            f"{int(row.hour):02d}:00" for row in peak_times_result.fetchall()
        ]
        ...
        logger.error(
            "Failed to retrieve EEG analytics",
            user_id=current_user.id,
            error=str(e)
        )
        raise HTTPException(
            status_code=500,
            detail="Failed to retrieve EEG analytics"
        )
    ...
    try:
        ...
        for row in rows:
            analytics.append(ContentAnalytics(
                ...
                recommendation_success_rate=round((row.recommendation_success_rate or 0) * 100, 2)
            ))
...
async def get_dashboard_data(
    ...
    try:
        ...
        learning_goals = [
            {"goal": "Complete 5 math lessons", "progress": 60, "target_date": "2025-10-01"},
            {"goal": "Improve attention score by 20%", "progress": 75, "target_date": "2025-09-30"}
        ]
