...
class RecommendationService:
        ...
        try:
            async with get_db() as db:
                ...
                if not available_content:
                    self.logger.warning("No available content for recommendations", user_id=user_id)
                    return []
        ...
        try:
            ...
            for session in sessions:
                ...
                if subject not in subject_performance:
                    subject_performance[subject] = {'scores': [], 'completed': 0, 'total': 0}
            ...
            for content_type in content_type_performance:
                ...
                if scores:
                    content_type_performance[content_type]['avg_score'] = np.mean(scores)
            ...
            return {
                ...
                'avg_score': np.mean([s.score for s in sessions if s.score]) if sessions else 0
            }

        except Exception as e:
            self.logger.error(
                "Failed to get user learning history",
                user_id=user_id,
                error=str(e)
            )
            return {}
    ...
    async def _get_available_content(
        ...
        try:
            ...
            params = {
                "user_id": int(user_id),
                "since": datetime.utcnow() - timedelta(days=30)  # Don't recommend recently completed content
            }
            ...
            self.logger.error(
                "Failed to get available content",
                user_id=user_id,
                error=str(e)
            )
            return []
    ...
    def _calculate_recommendation_score(
        ...
        if subject_perf:
            ...
            completion_rate = subject_perf.get('completed', 0) / subject_perf.get('total', 1)
        ...
        content_type = content['content_type']
        type_perf = user_history.get('content_type_performance', {}).get(content_type, {})
        ...
        if context:
            ...
            if any(keyword in content['title'].lower() or
                   keyword in ' '.join(content.get('tags', [])) for keyword in
                   ['beginner', 'introduction', 'basics'] if 'beginner' in context_lower):
                score += 1.0
                reasons.append("Matches beginner learning goal")

            if any(keyword in content['title'].lower() for keyword in
                   ['advanced', 'expert', 'master'] if 'advanced' in context_lower):
                score += 1.0
    ...
    def _apply_cognitive_load_filtering(
        ...
        cognitive_load_level = cognitive_load_prediction.get('cognitive_load_level', 'moderate')
        ...
        load_based_adjustment = {
            'very_low': {'max_difficulty': 5, 'min_difficulty': 3, 'reason': 'Very low cognitive load - suitable for challenging content'},
            'low': {'max_difficulty': 4, 'min_difficulty': 2, 'reason': 'Low cognitive load - moderate to challenging content appropriate'},
            'moderate': {'max_difficulty': 3, 'min_difficulty': 2, 'reason': 'Moderate cognitive load - balanced difficulty optimal'},
            'high': {'max_difficulty': 2, 'min_difficulty': 1, 'reason': 'High cognitive load detected - easier content recommended'},
            'very_high': {'max_difficulty': 1, 'min_difficulty': 1, 'reason': 'Very high cognitive load - basic content to prevent overload'}
        }

        adjustment_params = load_based_adjustment.get(cognitive_load_level, load_based_adjustment['moderate'])

        for content in available_content:
            content_difficulty = content.get('difficulty', 3)

            # Apply cognitive load-based filtering
            if adjustment_params['min_difficulty'] <= content_difficulty <= adjustment_params['max_difficulty']:
                ...
                if cognitive_load_level in ['very_low', 'low'] and content_difficulty >= 4:
                    content_copy['cognitive_boost'] = 0.2  # 20% boost for challenging content when load is low
                elif cognitive_load_level in ['high', 'very_high'] and content_difficulty <= 2:
                    content_copy['cognitive_boost'] = 0.15  # 15% boost for easy content when load is high
