...
class DynamicDifficultyService:
    ...
    async def adjust_difficulty(
        ...
        try:
            ...
            adjustment.research_validation = self._get_research_validation(cognitive_load, performance_metrics)
    ...
    def _assess_cognitive_load(self, eeg_features: Dict[str, float]) -> CognitiveLoad:
        ...
        theta_alpha_frontal = eeg_features.get('theta_alpha_ratio_frontal', theta_alpha_ratio)

        # Use frontal ratio if available (research emphasis)
        primary_ratio = theta_alpha_frontal if theta_alpha_frontal > 0 else theta_alpha_ratio
    ...
    def _evaluate_performance(self, learning_context: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        fatigue_factor = min(1.0, session_duration / 45)  # 45 minutes max before fatigue
    ...
    def _generate_adjustment_recommendation(
        ...
        confidence_factors = [
            ...
            1 if performance_metrics['performance_score'] > 0 else 0,  # Valid performance
            1 if eeg_features.get('signal_quality', 0) > 0.5 else 0,  # Good signal quality
        ]
        ...
        if difficulty_change != 0:
            expected_improvement = abs(difficulty_change) * 0.15  # Research-estimated improvement per level
        else:
            expected_improvement = 0.05  # Small improvement from maintaining optimal difficulty
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'p95_response_time_ms': sorted(self.response_times)[int(len(self.response_times) * 0.95)],
            'target_latency_achieved': sum(1 for t in self.response_times if t < 50) / len(self.response_times),
            'research_targets': {
